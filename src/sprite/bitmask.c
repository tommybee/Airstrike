...
{
  bitmask *temp = (struct bitmask *) malloc(sizeof(bitmask));
  if (! temp) return 0;
  temp->w = w;
  temp->h = h;
  temp->bits = (unsigned long *) calloc(h*((w - 1)/BITW_LEN + 1),sizeof(BITW));
  if (! temp->bits) 
    {
      free(temp);
      return 0;
...
static unsigned long bitpos[256];
...
void bitpos_setup()
{
  ...
  for (x=0;x<256;x++)
	...
	if (x & (1 << i))
      ...
      bitpos[x] = sum + ((n*8 + sum) << 8) +
	((n*16 + sum) << 16) + ((n*24 + sum) << 24);
    }
}
