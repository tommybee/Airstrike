...
{
  sprite_group_t *l;
  l = (sprite_group_t *) malloc(sizeof(sprite_group_t));
  l->nr_sprites = 0;
  l->max_sprites = 32; /* Start with an initial max of 32 */
  l->sprites = (sprite_t **) malloc(sizeof(sprite_t *)*(l->max_sprites));
  return l;
}
...
void sprite_group_insert(sprite_group_t *group, sprite_t *s)
{
  if (group->nr_sprites >= group->max_sprites)
    {
      CRITICAL(group->sprites = (sprite_t **) realloc(group->sprites,2*group->max_sprites*sizeof(sprite_t *)));
      group->max_sprites *= 2;
    }
